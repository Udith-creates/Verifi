...
void CreditScore_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void CreditScore_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 79;}

uint get_number_of_components() {return 4;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 73;}
...
ctx->componentMemory[coffset].templateName = "CreditScore";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_mul(&expaux[2],&signalValues[mySignalStart + 2],&circuitConstants[5]); // line circom 17
Fr_add(&expaux[1],&expaux[2],&signalValues[mySignalStart + 3]); // line circom 17
Fr_mul(&expaux[2],&signalValues[mySignalStart + 4],&circuitConstants[6]); // line circom 17
Fr_sub(&expaux[0],&expaux[1],&expaux[2]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
{
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[2]); // line circom 25
}}
...
CreditScore_3_create(1,0,ctx,"main",0);
CreditScore_3_run(0,ctx);
}
